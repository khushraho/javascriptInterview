...
async function convertStructure(originalJson) {
    const convertedJson = {};
    ...
    for (const key in originalJson) {
        ...
        if (originalJson[key].suggession) {
            convertedJson[key].suggession = await Promise.all(originalJson[key].suggession.map(async (suggestion) => {
                return {
                    "reply":{
...
(async () => {
    const convertedJson = await convertStructure(originalJson);
